...
if page == "About Data":
    ...
    about_tabs = st.tabs(["Overview", "Input Variables", "Global Oyster Harvesting Areas"])
...
# ==================================================
    ...
    with control_cols[0]:
        if st.button("▶ Start Animation", key="start_btn_common"):
            st.session_state["sim_playing"] = True
            st.rerun()
    with control_cols[1]:
        if st.button("⏸ Pause Animation", key="pause_btn_common"):
            st.session_state["sim_playing"] = False
            st.rerun()
    ...
    if {"Lat", "Long"}.issubset(df_view.columns):
        ...
        if open_fullscreen:
            # Create complete HTML for download/full screen with proper animation support
            ...
            # Provide download button for full interactive HTML
            st.download_button(
                label="📥 Download Full Screen Interactive Map",
                data=complete_html,
                file_name="norovirus_spatial_outbreak_map.html",
                mime="text/html",
                key="download_fullscreen_map"
            )
            st.info("✅ Downloaded file will have fully functional Play/Pause buttons and animations!")
